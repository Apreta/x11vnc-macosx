...
#
# Please read ALL of this instructional text at the top of this file
# before implementing this service.  If you don't do all of the require
# things described here it is unlikely the service will work.
#
...
# See below where you should put the VNC viewer jar file on your
# webserver.
#
...
# 
# NOTE: By default if the user does not have ~/.dmrc ~/.xsession or
# ~/.xinitrc files x11vnc will only start up a failsafe xterm session
# (where the user would then type something to start his desktop,
# e.g. gnome-session.)  In this case, you will need to set something
# like -env FD_PROG=/usr/bin/gnome-session or -env FD_SESS=gnome to have
# something besides the failsafe terminal at startup.
# 
...
#
# To have a per host conf put them in desktop.cgi.conf.hostname where
# "hostname" is for the machine with matching /bin/hostname output.
#
##########################################################################
# x11vnc:
#
...
# Xdummy:
#
# Xdummy is a hack replacement to Xvfb provided at the x11vnc website.
# It runs the Xorg server with the 'dummy' driver to provide a somewhat
# better virtual X server.  The main advantage to Xdummy is that the
# user can change the screen resolution dynamically (XRANDR extension.)
# 
# To do this, download the Xdummy script and run it via 'Xdummy -install'
# to install it.  Add -xdummy to your $x11vnc_extra_opts setting and make
# sure Xdummy is available in $PATH.
# 
##########################################################################
# Apache httpd:
#
...
#-------------------------------------------------------------------------
# Copyright (c) 2010-2012 by Karl J. Runge <runge@karlrunge.com>
#
...
##########################################################################
# Some sleep shims for starting the desktop:
#
my $sleep_shim1_noflag = 2;
my $sleep_shim2_noflag = 2;
my $sleep_shim1_flag = 8;
my $sleep_shim2_flag = 0.5;


##########################################################################
# Let x11vnc use a flag file to communicate with scripts and desktop.cgi
#
my $x11vnc_use_flag = 1;


##########################################################################
# Way to put x11vnc into the background. '&' or '-bg';
# -bg does not seem to work.
#
my $x11vnc_bg = '&';
...
##########################################################################
# Use /usr/bin/env to pass UNIXPW_CMD
#
# Some setups scrub the environment for httpd sub processes and
# so we need to use /usr/bin/env to set them.  The one below
# is for the activities surrounding UNIXPW_CMD running.
#
my $use_env_to_pass_unixpw_cmd = 0;
...
my $applet_html = <<"END";
<html>
<TITLE>x11vnc desktop (_UID_/_VNC_PORT_)</TITLE>
...
if (-f "$0.conf.$hostname") {
	eval `cat '$0.conf.$hostname'`;
}
...
# Check for 0.9.14 which has new features we can use:
#
my $x11vnc_have_0_9_14 = 0;
$needed    =      0 * 100 * 100 +      9 * 100 +     14;

if ($level >= $needed) {
	$x11vnc_have_0_9_14 = 1;
}
...
for (my $i = 0; $i < 8; $i++) {
	$my_pass_str .= $abc[ rand(scalar(@abc)) ];
}

# Use x11vnc trick to switch to user and store vnc pass in the passwdfile.
# Result is $my_pass_str is placed in user's $HOME/.x11vnc.pw
#
...
#
# On x11vnc 0.9.14 and later we will use the %pipe method to avoid
# this problem.
#
if (! $x11vnc_have_0_9_14) {
	copy_password_to_user($my_pass_str);
}
...
if (!$x11vnc_have_0_9_14) {
	...
	# Since the script will be piped into /bin/sh -s in this case
	# we put the password storing in the script and just call x11vnc
	# -unixpw once.  We assume echo is a builtin and will not be
	# viewable via ps.
	#
	my $upw = '$HOME/.x11vnc.pw';
	$mkpass = "touch $upw; chmod 600 $upw; echo '$my_pass_str' > $upw;";
}

# set up the flag file communication channel:
#
my $flag_file_pre = '';
my $sleep_post = '';
my $flag_options = '';
my $perl = '';
if ($x11vnc_use_flag) {
	foreach my $exe (qw(/usr/bin/perl /bin/perl /usr/local/bin/perl)) {
		if (-x $exe) {
			$perl = $exe;
			last;
		}
...
if (! $perl || ! $x11vnc_have_0_9_14) {
	$x11vnc_use_flag = 0;
}
my $shim1 = $sleep_shim1_noflag;
my $shim2 = $sleep_shim2_noflag;
if ($x11vnc_use_flag) {
	$shim1 = $sleep_shim1_flag;
	$shim2 = $sleep_shim2_flag;
	my $ff = '$HOME/.x11vnc.flag';
	$flag_options = "-flag $ff -rmflag nocreate:$ff";
	$flag_file_pre = "rm -f $ff;";
	$sleep_post = "$perl -e '"; 
	$sleep_post .= '$dt = 0.1; $T = shift; $F = shift; $t = 0; while ($t < $T) {last if -f $F; $t += $dt; select(undef, undef, undef, $dt);} unlink $F; exit 0;';
	$sleep_post .= "' $shim1 $ff;";
} else {
	$sleep_post = "sleep $shim1;";
}


# The x11vnc command script:
#
my $script = <<"END";
#!/bin/sh
...
$x11vnc -sigpipe ignore:HUP -nopw -rfbport '$vnc_port' \\
    ...
    $flag_options -timeout '$timeout' $extra $x11vnc_extra_opts \\
        >/dev/null 2>/dev/null </dev/null $x11vnc_bg
$sleep_post
exit 0;
END
$script .= "\n" x 16;
...
# We use %stdin before 0.9.14 and %pipe after:
#
my $unixpw_mode = '%stdin';
if (! $x11vnc_have_0_9_14) {
	print TMP $script;
} else {
	$ENV{UNIXPW_CMD} = "/bin/sh -s";
	$unixpw_mode = '%pipe';
}
close TMP;
...
# Check if we must use /usr/bin/env:
#
my $pre = '';
if ($use_env_to_pass_unixpw_cmd) {
	$pre = "/usr/bin/env UNIXPW_CMD='$ENV{UNIXPW_CMD}'";
}

# Start the x11vnc cmd:
#
if (!open(X11VNC, "| $pre $x11vnc -unixpw $unixpw_mode > /dev/null")) {
	unlink $tmpfile;
...
print X11VNC "$username:$password\n";
if ($x11vnc_have_0_9_14) {
	# %pipe mode, we pipe this script to /bin/sh -s:
	#
	print X11VNC length($script), "\n";
	print X11VNC $script;
}
close X11VNC;	# note we ignore return value.
unlink $tmpfile;

# Sleep a little bit just to be sure x11vnc starts listening
#
fsleep($shim2);

if ($rmlock) {
	# let our x11vnc proceed even more before removing lock.
	sleep 2;
	unlink $rmlock;
}
...
sub copy_password_to_user {
	...
	# Start it:
	#
	if (!open(X11VNC, "| $pre $x11vnc -unixpw \%stdin > /dev/null")) {
		unlink $fifo;
