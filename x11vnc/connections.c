...
int check_access(char *addr) {
	...
	/* sanity test string from libvncserver is set to something: */
	if (addr == NULL || *addr == '\0') {
		...
		if (nb == 0) {
			rfbLog("check_access: denying blank host IP address string.\n");
			return 0;
		}
	}
	...
	if (*allow_list == '\0' && allow_once == NULL) {
		/* no constraints, so accept it */
		return 1;
	}

	if (strchr(allow_list, '/') && !strstr(allow_list, "unix=")) {
	...
	while (p) {
		...
		if (!strcmp(addr, "NON_SOCKET"))	known_label = 1;
		if (!strcmp(addr, "NAMEFAIL_AF_UNIX"))	known_label = 1;
		if (!strcmp(addr, "UNNAMED_AF_UNIX"))	known_label = 1;
		if (!strcmp(addr, "UNKNOWN_AF"))	known_label = 1;

		if (known_label) {
			chk = p;
			rfbLog("check_access: checking '%s' against full string '%s' for known name label\n", addr, chk);
			fullmatch = 1;
		} else if (!strcmp(addr, p)) {
			chk = p;
			rfbLog("check_access: checking '%s' against full string '%s' (since we see they are equal)\n", addr, chk);
			fullmatch = 1;
		} else if (strstr(p, "unix=") == p) {
			chk = p + strlen("unix=");
			rfbLog("check_access: checking '%s' against unix socket string '%s'\n", addr, chk);
			fullmatch = 1;
		} else if (ipv6_ip(p)) {
			chk = p;
			...
			r = host2ip(p);
			if (r == NULL || *r == '\0') {
				rfbLog("check_access: failed lookup '%s', skipping check.\n", p);
				p = strtok(NULL, ", \t\n\r");
				continue;
			}
		...
		if (getenv("X11VNC_DEBUG_ACCESS")) rfbLog("check_access: debug: chk: '%s'  part: '%s'  addr: '%s'\n", chk, p, addr);

		if (!fullmatch && getenv("X11VNC_ALLOW_FULLMATCH")) {
			rfbLog("check_access: doing full string matches only (enabled by user set X11VNC_ALLOW_FULLMATCH)\n");
			fullmatch = 1;
		} else if (fullmatch) {
			rfbLog("check_access: doing full string matches only (enabled by context.)\n");
		}

		q = strstr(addr, chk);	/* q may be NULL */
		if (fullmatch) {
			if (!strcmp(addr, chk)) {
				if (chk != p) {
					rfbLog("check_access: client '%s' fullmatch matches host '%s' (%s)\n", addr, chk, p);
				} else {
					rfbLog("check_access: client '%s' fullmatch matches host '%s'\n", addr, chk);
				}
				allowed = 1;
			} else {
				if (!strcmp(chk, "localhost") || !strcmp(chk, "127.0.0.1") || !strcmp(chk, "::1")) {
					if (!strcmp(addr, "127.0.0.1") || !strcmp(addr, "::1")) {
						rfbLog("check_access: client '%s' fullmatch is local (check: %s)\n",
						    addr, chk);
						allowed = 1;
					}
				}
			}
		} else if (ipv6_ip(addr)) {
			if (!strcmp(chk, "localhost") && !strcmp(addr, "::1")) {
				...
				rfbLog("check_access: client '%s' matches starting pattern '%s'\n", addr, chk);
				allowed = 1;
			}
...
void check_ipv6_listen(long usec) {
	...
	if (ipv6_listen_fd >= 0 && FD_ISSET(ipv6_listen_fd, &fds)) {
		...
		if (name) {
			if (cl->host) {
				rfbLog("ipv6 client: old client name: '%s'\n", cl->host);
				free(cl->host);
			}
			cl->host = name;
...
void check_unix_sock(long usec) {
	...
	if (unix_sock_fd < 0 && strstr(unix_sock, "fd=") == unix_sock) {
		int fd = -1;
		if (!strcmp(unix_sock, "fd=0")) {
			fd = 0;
		} else {
			char *s = unix_sock + strlen("fd="); 
			if (atoi(s) <= 0) {
				rfbLog("invalid fd=n unixsock: %s\n", unix_sock);
				clean_up_exit(1);
			}
			fd = atoi(s);
		}
		rfbLog("unixsock: connecting to client on preexisting fd=%d\n", fd);
		rfbLog("unixsock: F_GETFL(%d):  0x%x\n", fd, fcntl(fd, F_GETFL));
		if (!create_new_client(fd, 1)) {
			rfbLog("unixsock: error creating fd=%d client.\n", fd);
			clean_up_exit(1);
		}
		free(unix_sock);
		unix_sock = NULL;
...
void client_set_net(rfbClientPtr client) {
	...
	rfbLog("client useCopyRect: %s %d\n", client->host, client->useCopyRect);
	cd = (ClientData *) client->clientData;
	if (cd == NULL) {
		return;
	}
...
void send_client_info(char *str) {
	...
	static int len = 128, dbg = -1; 

	if (dbg < 0) {
		dbg = 0;
		if (getenv("X11VNC_DEBUG_SEND_CLIENT_INFO")) {
			dbg = 1;
		}
	}
	...
	if (dbg) {
		rfbLog("send_client_info: len0: %d len: %d strlen: %d str:\n'%s'\n",
		    len0, len, strlen(str), str);
	}
