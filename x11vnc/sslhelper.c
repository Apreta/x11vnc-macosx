...
static int pem_passwd_callback(char *buf, int size, int rwflag,
	...
	if (getenv("X11VNC_PEMPW")) {
		char *s = getenv("X11VNC_PEMPW"); 
		if (strlen(s) > sizeof(line) - 10) {
			exit(1);
		}
		if (strstr(s, "file:") == s) {
			FILE *f = fopen(s + strlen("file:"), "r");
			if (!f) {
				exit(1);
			}
			...
			fclose(f);
		} else if (strstr(s, "rmfile:") == s) {
			FILE *f = fopen(s + strlen("rmfile:"), "r");
			if (!f) {
				exit(1);
			}
			unlink(s + strlen("rmfile:"));
			if(fgets(line, 1024, f) == NULL) {
				exit(1);
			}
			fclose(f);
		...
		memset(s, 0xff, strlen(s));
		set_env("X11VNC_PEMPW", "--------------------------------------");
	} else {
...
static int watch_for_http_traffic(char *buf_a, int *n_a, int raw_sock) {
	...
	*n_a = 0;
	...
	if (enc_str && !strcmp(enc_str, "none")) {
     ...
     if (err == SSL_ERROR_NONE && n == 1) {
	if (db) fprintf(stderr, "watch_for_http_traffic only read one byte: %c\n", buf[0]);
	usleep(200*1000);
	if (enc_str && !strcmp(enc_str, "none")) {
		n = read(raw_sock, buf+1, 1);
		err = SSL_ERROR_NONE;
	} else {
#if LIBVNCSERVER_HAVE_LIBSSL 
		n = SSL_read(ssl, buf+1, 1);
		err = SSL_get_error(ssl, n);
#else
		err = n = 0;
...
#else
		n2 = 0;
		badnews("3 in watch_for_http_traffic");
#endif
	}
	if (n2 >= 0) {
		n += n2;
