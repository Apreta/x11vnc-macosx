...
int su_verify(char *user, char *pass, char *cmd, char *input, char *rbuf, int *rbuf_size, int nodisp);
...
int su_verify(char *user, char *pass, char *cmd, char *input, char *rbuf, int *rbuf_size, int nodisp) {
	...
	if (getenv("X11VNC_SLOW_PW_MS")) {
		slow_pw_ms = atoi(getenv("X11VNC_SLOW_PW_MS"));
	}
...
int unixpw_verify(char *user, char *pass) {
	...
	if (unixpw_cmd) {
		...
		if (su_verify(user, pass, NULL, NULL, NULL, NULL, 1)) {
