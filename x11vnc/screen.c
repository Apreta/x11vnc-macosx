...
static rfbClient* rfb_client = NULL;
...
void vnc_reflect_got_cursorshape(rfbClient *cl, int xhot, int yhot, int width, int height, int bytesPerPixel) {
	...
	red_mask   = (rfb_client->format.redMax   << rfb_client->format.redShift);
	green_mask = (rfb_client->format.greenMax << rfb_client->format.greenShift);
	blue_mask  = (rfb_client->format.blueMax  << rfb_client->format.blueShift);
	...
	for (j=0; j<height; j++) {
		for (i=0; i<width; i++) {
			...
			int m;
			if (bytesPerPixel == 1) {
				unsigned char* p = (unsigned char *) rfb_client->rcSource;
				ui = (unsigned int) *(p + j * width + i);
			} else if (bytesPerPixel == 2) {
				unsigned short* p = (unsigned short *) rfb_client->rcSource;
				ui = (unsigned int) *(p + j * width + i);
			} else if (bytesPerPixel == 4) {
				unsigned int* p = (unsigned int *) rfb_client->rcSource;
				ui = (unsigned int) *(p + j * width + i);
			}
			r = (red_mask   & ui) >> rfb_client->format.redShift;
			g = (green_mask & ui) >> rfb_client->format.greenShift;
			b = (blue_mask  & ui) >> rfb_client->format.blueShift;

			r = (255 * r) / rfb_client->format.redMax;
			g = (255 * g) / rfb_client->format.greenMax;
			b = (255 * b) / rfb_client->format.blueMax;

			ui = (r << 16 | g << 8 | b << 0) ;

			uic = (unsigned char *)rfb_client->rcMask;
			m = (int) *(uic + j * width + i);
			if (m) {
				ui |= 0xff000000;
			}
...
static void from_libvncclient_CopyRectangleFromRectangle(rfbClient* rfb_client, int src_x, int src_y, int w, int h, int dest_x, int dest_y) {
  int i,j;

#define COPY_RECT_FROM_RECT(BPP) \
  { \
    uint##BPP##_t* _buffer=((uint##BPP##_t*)rfb_client->frameBuffer)+(src_y-dest_y)*rfb_client->width+src_x-dest_x; \
    if (dest_y < src_y) { \
      for(j = dest_y*rfb_client->width; j < (dest_y+h)*rfb_client->width; j += rfb_client->width) { \
        if (dest_x < src_x) { \
          for(i = dest_x; i < dest_x+w; i++) { \
            ((uint##BPP##_t*)rfb_client->frameBuffer)[j+i]=_buffer[j+i]; \
          } \
        } else { \
      ...
      for(j = (dest_y+h-1)*rfb_client->width; j >= dest_y*rfb_client->width; j-=rfb_client->width) { \
        if (dest_x < src_x) { \
          for(i = dest_x; i < dest_x+w; i++) { \
            ((uint##BPP##_t*)rfb_client->frameBuffer)[j+i]=_buffer[j+i]; \
          } \
  ...
  switch(rfb_client->format.bitsPerPixel) {
...
static rfbCredential* vnc_reflect_get_credential(rfbClient* rfb_client, int type) {
...
static char* vnc_reflect_get_password(rfbClient* rfb_client) {
...
int rawfb_vnc_reflect_init_client(void) {
	return rfbInitClient(rfb_client, &client_argc, client_argv);	
}
...
char *vnc_reflect_guess(char *str, char **raw_fb_addr) {
	...
	if (rfb_client == NULL) {
		...
		rfb_client = rfbGetClient(bitsPerSample, samplesPerPixel, bytesPerPixel);
	}
	...
	rfb_client->HandleCursorPos = vnc_reflect_cursor_pos;
	rfb_client->GotFrameBufferUpdate = vnc_reflect_got_update;
	rfb_client->MallocFrameBuffer = vnc_reflect_resize;
	rfb_client->Bell = vnc_reflect_bell;
#if 0
	rfb_client->SoftCursorLockArea = NULL;
	rfb_client->SoftCursorUnlockScreen = NULL;
	rfb_client->FinishedFrameBufferUpdate = NULL;
	rfb_client->HandleKeyboardLedState = NULL;
	rfb_client->HandleTextChat = NULL;
#endif
	rfb_client->GotXCutText = vnc_reflect_recv_cuttext;
	rfb_client->GotCursorShape = vnc_reflect_got_cursorshape;
	rfb_client->GotCopyRect = vnc_reflect_got_copyrect;

	if (getenv("X11VNC_REFLECT_PASSWORD")) {
		rfb_client->GetPassword = vnc_reflect_get_password;
	}
#ifdef rfbCredentialTypeX509
	rfb_client->GetCredential = NULL;
	if (0 || getenv("LIBVNCCLIENT_GET_CREDENTIAL")) {
		rfb_client->GetCredential = vnc_reflect_get_credential;
	}
#endif
	...
	if (first) {
		...
		if (strstr(hp, "listen") == hp) {
			...
			if (q) {
				rfb_client->listenPort = atoi(q+1);
			} else {
	...
	if (at) {
		...
		sprintf(str2, "map:/dev/null@%dx%dx%d:0x%lx/0x%lx/0x%lx",
		    w, h, rfb_client->format.bitsPerPixel,
		    red_mask, green_mask, blue_mask);
	}
	*raw_fb_addr = (char *) rfb_client->frameBuffer;
	free(str0);
...
rfbBool vnc_reflect_send_pointer(int x, int y, int mask) {
	...
	return SendPointerEvent(rfb_client, x, y, mask);
}
...
rfbBool vnc_reflect_send_key(uint32_t key, rfbBool down) {
	...
	return SendKeyEvent(rfb_client, key, down);
}
...
rfbBool vnc_reflect_send_cuttext(char *str, int len) {
	...
	return SendClientCutText(rfb_client, str, len);
}
...
void vnc_reflect_process_client(void) {
	...
	num = WaitForMessage(rfb_client, 1000);
	if (num > 0) {
...
void flag_file_activities(void) {
	static int first = 1;
	if (flagfile) {
		...
		if (flag) {
			...
			fflush(flag);	
			fclose(flag);
			rfbLog("created flag file: %s with PORT=%d SSL_PORT=%d\n",
			    flagfile, screen->port, stunnel_port);
		} else {
