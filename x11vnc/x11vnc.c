...
static void quick_pw(char *str) {
	...
	char tmp[1024];
	int db = 0;
	char *cmd_input = getenv("UNIXPW_CMD_STDIN");
	...
	if (!strcmp(str, "%-") || !strcmp(str, "%stdin")) {
		...
		if (cmd_input == NULL) {
			...
			if (len == -1) {
				char tmp2[8192];
				/* one line */
				if(fgets(tmp2, 8192, stdin) == NULL) {
					exit(2);
				}
				len = strlen(tmp2);
				if (len > 0) {
					cmd_input = (char *) calloc(len + 10, 1);
					strcat(cmd_input, tmp2);
				}
			} else if (len > 0) {
				int n;
				/* read len bytes from stdin */
				cmd_input = (char *) calloc(len + 10, 1);
				n = fread(cmd_input, 1, len, stdin);
				if (n != len) {
					exit(2);
				}
			}
	...
	*q = '\0';
	...
	if (unixpw_cmd) {
		...
		if (su_verify(p, q+1, ucmd, cmd_input, NULL, NULL, 1)) {
			fprintf(stdout, "Y %s\n", p);
			exit(0);
		} else {
			fprintf(stdout, "N %s\n", p);
...
static void check_guess_auth_file(void)  {
	if (!strcasecmp(auth_file, "guess")) {
		...
		if (!strcmp(line, "")) {
			...
			rfbLog("-auth guess: using '%s' for DISPLAY='%s'\n", line, disp);
...
#	define CHECK_ARGC if (i >= argc-1) { \
	...
	for (i=1; i < argc; i++) {
		...
		if (!strcmp(arg, "-macdisplay")) {
			CHECK_ARGC
			macosx_display = atoi(argv[++i]);
			continue;
		}
		if (!strcmp(arg, "-gui")) {
			launch_gui = 1;
	...
	if (dpy)  {
		/* Xorg broke XTestFakeMotionEvent() on multiple screens in 2009. */
		rfbLog("Number of screens: %d for X11 display '%s'\n", ScreenCount(dpy), DisplayString(dpy));
		if (ScreenCount(dpy) > 1 && !use_xwarppointer && !got_noxwarppointer) {
			rfbLog("Multiscreens: screen count: %d  my screen: %d\n", ScreenCount(dpy), DefaultScreen(dpy));
			rfbLog("Multiscreens: enabling -xwarppointer mode to try to correct\n");
			rfbLog("Multiscreens: pointer motion: XTestFakeMotionEvent Xorg bug.\n");
			rfbLog("Multiscreens: Use -noxwarppointer to force XTEST.\n");
			use_xwarppointer = 1;
		}
	}
